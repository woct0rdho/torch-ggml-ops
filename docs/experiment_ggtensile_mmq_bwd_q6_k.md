...
1. Match HIP M128/N64/K32 ownership for M256 and M128; use two exact M tiles for M256.
2. Add exact M64/N32/K64 ownership if the initial representable geometry leaves a material margin.
3. Compare packed versus scalar extraction and low/high/scale load grouping.
4. Compare HIP-analogous XOR8/XOR16 layouts with unswizzled padding and unpadded controls.
5. Measure decoder-row ownership, payload lane sharing, and metadata broadcast.
6. Test one versus two decoded-B buffers only if lower bounds expose actionable overlap.
7. Test DepthU, PGR/PLR, SIA, bounded next-tile packed prefetch, and address-state lifetime.
8. Test WGM only where more than one exact M tile exists and launch ordering can matter.
