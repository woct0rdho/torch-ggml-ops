...
The completed campaign closures remain valid for the exact packed bodies, geometries, schedules, comparators, and compiler artifacts that were measured. They are not a blanket closure over later source-level review of the typed GGTensile writers. That review found a small set of still-unqualified probes: paired zero-bank hoisting for IQ2_S, IQ2_XXS, and Q3_K; IQ2_XXS sign-selector fusion and pre-scaled `d`; Q3_K variable-offset BFE extraction; partial `v94:v99` initialization in non-paired IQ2_S; paired epilogue setup and exact BF16 scheduling; serial IQ2_XXS J80 geometry; and isolated processor-mode, clause, delay-ALU, and bank-valid VOPD artifact screens. Their source instruction estimates are qualification inputs, not timing results, and their family records are authoritative.

The ordinary decoded-LDS Q2_K, Q4_K, and Q5_K bodies already initialize their zero banks once per row tile, so the paired lifetime finding does not transfer to them. Their measured VOPD, geometry, route, address, epilogue, and resource closures remain scoped to their existing premises. Backward established that direct native BF16-C is numerically invalid for long reductions, K32/K64 FP32 slabs increase resources and latency, and normalized FP16-C remains 33.3% slower even when scale calculation is removed. Forward approximate accumulation, wider J tiles based on reclaimed registers, and static persistent scheduling are therefore still closed at their prerequisites.

Reopen that sequence only after a new arithmetic mechanism demonstrates lower register use, acceptable real-weight and dynamic-range error, zero private storage and spills, and better public backward latency in an existing N64 body. Forward must then measure its existing geometry first, preserve integer MMQ and Q8_1 workspace semantics, and test only DeepSeek Q2_K J64, the serial DeepSeek IQ2_XXS J64/J80 identities, or Qwen IQ2_S J128 where the latest B16 results justify it. Static grids from `{20,40,80,160,256}` come last.
...
The remaining losses are representation-level comparisons against already decoded BF16 weights. Reduced-precision accumulation is also closed by the completed backward controls. No broad local tile or representation sweep is planned. The bounded source-level probes listed above remain unqualified until they pass their family-specific artifact, resource, exactness, deterministic-build, and warmed complete-call/prequantized timing gates. Any actionable result reopens the affected record and requires the complete recursive final review from the changed premise before it can be considered closed. A compact reusable decoded representation, a transient dense stage, or a new profile-supported arithmetic mechanism remains the larger end-to-end path beyond these probes.
