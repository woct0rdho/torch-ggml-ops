...
The selected source is generated from `mmq_fwd_q3_k_inventory.json` and `mmq_fwd_q3_k_solutions.json`. The bundled HSACO is byte-identical to the independently qualified artifact. Inspection reports code-object v5, gfx1151, wave32, the 40-byte ABI, 144 VGPRs, 16 SGPRs, 28,672 static LDS bytes, zero private storage, and zero spills.
