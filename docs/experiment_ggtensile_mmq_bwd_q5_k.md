...
- Six-key catalog and immutable campaign runner. `tools/ggtensile/configs/mmq_bwd_q5_k_catalog.json` maps each exact key directly to one of the four deduplicated current winners. Family labels, representative tensors, call weights, timing evidence, and the initial two-buffer history remain in this record and campaign reports rather than the deployment JSON.
...
## Post-Audit Relaxed-BF16 Transfer

Status: planned and unmeasured. The exact scalar, pipeline, high-bit, and geometry conclusions above remain closed. This reopening changes only the BF16 conversion contract and therefore does not contradict those measurements.

Use the site-separated `RNEPreserveNaN`, `BiasRound`, and `Truncate` policies defined in the Q4_K backward record. Keep decoded-weight LDS staging and final output conversion as independent solution fields and experiments. Screen the highest-absolute-cost narrow M32768 key first, followed by shared-down M32768 only if the first result removes at least one percent of body latency without changing resources. A Q4 result is implementation evidence, not a transferable Q5 selection; Q5 high-bit reconstruction, exact-key resources, error, and timing must be qualified independently.

Cross-record priority places this behind Q6 M64 and Q4 K512 staging. Q5 pays the same per-value conversion but surrounds it with additional high-bit reconstruction, so conversion is a smaller fraction of its decoder. The planning prior is low- to mid-single-digit body movement, with the narrow M32768 key retained as the discriminator because of its absolute cost. This is not a measured bound and does not permit inference from Q4 or Q6 timing.

Numerical tests must use finite inputs, fail on any non-finite output, and report error distributions against the exact Q5 parent and independent reference. No kernel-side NaN/Inf test or repair path is added. Mutation, reproducibility, resource, and shape-rejection gates remain unchanged. Any relaxed result stays outside the exact catalog and requires model-training integration before retention or deployment.
