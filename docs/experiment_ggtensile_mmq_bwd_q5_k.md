...
| Two-buffer SIA3/PGR1 | Runtime illegal memory access | Rejected as unsupported by the Q5 pipeline handoff. |
| Two-buffer DepthU64 | Incorrect against HIP | Rejected; current double-buffer decoder contract is DepthU32. |
...
The Q5 low-payload nibble-half shift is invariant within a decode phase, so a two-instruction hoist was implemented and passed all six-key producer-mutation checks. Its first matrix screen reached `0.98519` weighted candidate/control latency, with five exact keys improving. Isolated shared-down `M8192` rechecked at `1.06616`, so the hoist is selected for the other five exact keys and the original per-chunk path is selected for that key. A follow-on `v_lshl_or_b32` fusion replaced the high-plane shift and OR, reduced the selected N128 static VALU issue count to 920 (948 for the no-hoist M8192 key), and produced the final 25-repeat weighted candidate/HIP latency ratio of `0.76592`.
...
The final catalog uses the Q5 packed extraction path, `v_lshl_or_b32` high-plane fusion on all six keys, the nibble-shift hoist on five keys, and the original nibble-shift path for shared-down `M8192`. The authoritative protocol used 10 warmups, 25 rotating same-process repeats, serial phases, and HIP as the performance control.
...
The final recursive pass classified the remaining mechanisms as follows: one-buffer ownership, WGM variants, SIA5/store priority, scalar extraction, lane sharing, and vector metadata are measured rejections; SIA3/PGR1 and double-buffer DepthU64 are runtime/correctness failures; persistent workgroups, split-K, prepared weights, cache-policy ownership, and broader geometries expand the contract without a first-order lower-bound opportunity; native packed BF16 arithmetic and VOPD pairings are unavailable or non-bit-exact on gfx1151; and b128 payload loads, `v_lshl_or_b32`, exact-trip specialization, and the current WMMA/LDS ownership are retained. No new valid in-contract actionable mechanism remains after the follow-up review. The remaining bottleneck is WMMA/A/LDS for shared-down and overlapped WMMA/decode/LDS work for narrow, as shown by the floors above.
