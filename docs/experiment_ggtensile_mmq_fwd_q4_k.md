...
The next control gives each 128-thread workgroup a `128x64` macro tile. Four flat wave32 waves each reuse one Q4_K `16x32` weight fragment across eight activation-row tiles, reducing repeated weight decode and metadata work without LDS. A nine-repeat rotating screen measured 0.294560 ms for the prequantized body versus 0.174040 ms for HIP, a `1.6925x` ratio, and 0.316359 ms complete versus 0.197112 ms for HIP, a `1.6050x` ratio. It preserves the one-element `2.98e-8` HIP difference and exactly preserves HIP's independent-reference envelope: normalized RMSE `0.0136406` and maximum absolute error `0.046875`. This is a material improvement over the one-wave control but is not retainable because it remains slower than HIP. Its principal exposed redundancy is that all four waves independently load the same 128-row DS4 activation tile; the next mechanism stages that tile once in LDS.
