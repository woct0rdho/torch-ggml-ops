...
Q2_K participates in the common R1 packed-kernarg and R2 paired cumulative-offset experiments, but their transfer is gated by the shorter Q4_K B1 body. R3 receives an additional Q2_K B16 screen because the retained hardware counters identify instruction count, VALU issue cycles, instruction-fetch waits, and residency rather than DRAM or LDS stalls. The Q2 specialization removes only the multiply by the shape-guarded zero high expert-stride word and combines exact power-of-two BF16 output addressing; it does not change distributed decode, `F16_D2S6`, the persistent all-ones operands in `v28:v31`, correction association, waits, barriers, or epilogue policy.

The Q2 screen uses the selected J64 parent, the adjacent installed J32 control, deterministic 512-draw DeepSeek learned and hash confirmation banks with their 40/43 and 3/43 reporting weights, five weighted medoids per component, three warmups, and nine order-controlled complete-call repeats. The existing bitwise, resource, deterministic-build, mutation, and invalid-route gates apply. Reversed-order 25-repeat confirmation is required only if R3 clears the two-percent aggregate gate without a profile below `0.99x`.

The two independent build passes covered Q2_K R35 and all three production keys. R3 retained the selected J64 resources of 159 VGPRs, 40 SGPRs, 30,208 LDS bytes, 80 static WMMAs, and four barriers, with zero private storage and spills. It removed three emitted instructions, one artifact VALU issue, and 16 code-object bytes. The cross-format bounded matrix remained bitwise equal to the parent on first, odd/even non-first, repeated-ID, boundary, skewed, mutation, invalid-expert, and invalid-offset routes.
...
- Rejected: dynamic parents; standalone association; HIP packed 400-byte layout; 336-byte layout; full and descending two-group pipelines; compact next-group prefetch; partial VMEM; VOPD initialization; explicit producer and activation hard clauses; cache invalidations; priority zero; epilogue dependency widths one/four; J32/J16 two-tile and J64 one/two-tile epilogues; pure J32 selection; J64 for B1/B4; bitwise-invalid J64/J32 mixed tails; J128; common packed kernarg and paired-route loads; and guarded stride/output-address reduction. The J128 phased screen is exact but resource-limited at `239/40/39,424/160/4` and is slower than J64. The address reduction is exact and resource-neutral but regresses B16 complete-call time by 1.529%.
