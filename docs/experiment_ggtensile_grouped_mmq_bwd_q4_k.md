...
3. If fixed J128 tasks advance, compare one J64 endpoint or a single-launch mixed J128/J64 tail policy. Do not reopen N-major ordering, a second full/tail launch, fixed persistent traversal, or another global `SplitRoutes` sweep: those mechanisms already have direct negative controls.
4. Reconfirm the selected `DependencyBatch4` schedule under any retained ownership or suppression mechanism. It is already implemented and selected on all three Q4_K keys; it is an interaction control, not missing work.
5. Reopen scale/minimum decode sharing only if the ownership/tail changes expose a resource-clean lower-state design. Standalone metadata broadcast, dedicated decoder waves, approximate accumulators, Stream-K, XCC remapping, and generic prepared shadows remain closed by direct evidence or contract mismatch.
