...
def inspect_grouped_forward_pair_artifact(
    ...
    state = DerivedGroupedForwardPairState.from_problem_spec(problem, kernel_spec)
    row_tasks = (
    ...
    expected_metadata = {
        ...
        ".max_flat_workgroup_size": state.kernel_spec.geometry.work_group[0]
    ...
    assert _metadata_arguments(kernel) == abi.metadata_arguments
    ...
    expected_wmmas = 2 * state.kernel_spec.geometry.macro_tile[0]
    assert wmma_count == expected_wmmas
    assert barrier_count == 8
