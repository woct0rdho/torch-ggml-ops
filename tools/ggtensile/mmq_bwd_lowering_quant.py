...
from .iq2_xxs_grid import IQ2_XXS_GRID_BYTES, iq2_xxs_grid_rodata
...
IQ2_XXS_GRID_SYMBOL = ".LGGTensileIQ2XXSGrid"
...
class BackwardQuantLowering:
    ...
    def trailing_sections(self) -> tuple[str, ...]:
        ...
        if quant_type == "IQ2_XXS":
            return (iq2_xxs_grid_rodata(IQ2_XXS_GRID_SYMBOL),)
        return ()

    def _emit_quant_constants(self, asm: _Assembly) -> None:
        quant_type = self.state.contract.quant_type
        symbol = {
            "IQ2_S": IQ2_S_GRID_SYMBOL,
            "IQ2_XXS": IQ2_XXS_GRID_SYMBOL,
        }.get(quant_type)
        if symbol is None:
            return
    ...
    def _emit_quant_codebook_stage(self, asm: _Assembly) -> None:
        ...
        if codebook_bytes is None or not self.physical.lds.codebook_in_lds:
            return
        r = self.registers
        threads = self.state.spec.geometry.num_threads
        bytes_per_thread = codebook_bytes // threads
        loads_per_thread = bytes_per_thread // 16
        address = r.temporary
        lds_address = address + 1
        asm.comment(f"Stage the {quant_type} codebook once in disjoint LDS.")
        asm.inst(
    ...
    def _emit_quant_global_reads(
        ...
        *,
        ...
        if quant_type == "Q2_K":
            ...
            self._emit_iq2_xxs_global_reads(asm, wait_for_reads=wait_for_reads)
        else:
    ...
    def _emit_iq2_xxs_global_reads(
        ...
        *,
        ...
        asm.comment("Build IQ2_XXS block addresses for decoder-owned output rows.")
        asm.inst(f"v_lshrrev_b32 v{t}, {k_shift}, v{r.serial}")
        asm.inst(f"v_add_nc_u32 v{t}, s{r.loop_counter}, v{t}")
        asm.inst(f"v_mul_lo_u32 v{t}, {packed_row_bytes}, v{t}")
        asm.inst(f"v_add_nc_u32 v{t}, s{r.block_offset}, v{t}")
        ...
        asm.comment("Map each lane to one IQ2_XXS aligned 16-value group.")
        asm.inst(f"v_and_b32 v{t}, {n_tiles - 1}, v{r.serial}")
        asm.inst(f"v_lshlrev_b32 v{t}, 4, v{t}")
        asm.inst(f"v_add_nc_u32 v{t}, s{r.scalar_temporary}, v{t}")
    ...
    def _emit_q8_0_global_reads(
        ...
        *,
        ...
        for row in range(decoder_rows):
            ...
            if self.state.spec.decode.extraction is not BackwardExtraction.Scalar:
                asm.inst(
                    f"global_load_b128 v[{q + 4 * row}:{q + 4 * row + 3}], "
                    f"v{payload_address}, s[{r.kernarg + 2}:{r.kernarg + 3}]"
                )
    ...
    def _emit_q5_k_metadata_loads(
        ...
        r = self.registers
        if self.state.spec.decode.uses_vector_metadata_load:
    ...
    def _emit_quant_decode(
        ...
        *,
        ...
        if quant_type == "Q2_K":
            ...
            self._emit_iq2_xxs_decode(asm, label_suffix=label_suffix)
        else:
    ...
    def _emit_q2_k_decode_chunk(self, asm: _Assembly, chunk: int) -> None:
        ...
        decode_plan = self.physical.dependency_decode
    ...
    def _emit_q6_k_decode_prepare(
        ...
        *,
        ...
        n_shift = n_per_tile.bit_length() - 1
        t = r.temporary
        low_shift = self.physical.address.low_bitfield_shift
    ...
    def _emit_q6_k_decode_chunk(self, asm: _Assembly, chunk: int) -> None:
        ...
        asm.inst(
            f"v_lshrrev_b32 v{low}, v{self.physical.address.low_bitfield_shift}, v{low}"
        )
        ...
        extraction = self.state.spec.decode.extraction
        d_scaled = t + 1 + 2 * row
        value = t + 1 + 2 * decoder_rows
        if extraction is BackwardExtraction.PackedVopd:
    ...
    def _emit_q8_0_decode_chunk(self, asm: _Assembly, chunk: int) -> None:
        ...
        if self.state.spec.decode.extraction is BackwardExtraction.PackedVopd:
            rounding = value + 2
            alternate_d = value + 3
            if first_element == 0:
                asm.inst(f"v_mov_b32 v{alternate_d}, v{d_scaled}")
    ...
    def _emit_iq2_xxs_codebook_reads(
        ...
        assert self.physical.lds.codebook_in_lds
        r = self.registers
        t = r.temporary
    ...
    def _emit_iq2_xxs_decode_chunk(
        ...
        *,
        ...
        row = chunk // 4
        element_start = 4 * (chunk % 4)
        assert 0 < dword_count <= 4 - chunk % 4
        k_span = self.state.spec.geometry.depth_u // self.physical.decoder.rows
    ...
    def _emit_q3_k_decode_prepare(
        ...
        *,
        ...
        if self.physical.decoder.full_instruction_pairing:
            if decoder_rows == 1:
        ...
        asm.inst(f"v_lshlrev_b32 v{t + 1}, 1, v{t + 1}")
        asm.inst(f"v_mov_b32 v{self.physical.address.low_bitfield_shift}, v{t + 1}")
        asm.inst(f"v_lshrrev_b32 v{t}, 5, v{t}")
        asm.inst(f"v_mov_b32 v{quant_shift}, v{t}")
        if self.state.spec.decode.extraction is BackwardExtraction.Packed:
    ...
    def _emit_q3_k_decode_chunk(self, asm: _Assembly, chunk: int) -> None:
        ...
        if self.state.spec.decode.extraction is BackwardExtraction.Scalar:
            for element in range(first_element, first_element + 4):
                value = t + 5
                rounding = t + 6
                d_scaled = t + 3 + row if row < 2 else r.quant_dm + row
        ...
        q0 = t + 5
        q1 = t + 8
        full_vopd = self.physical.decoder.full_instruction_pairing
    ...
    def _emit_q45_metadata_prepare(
        ...
        *,
        ...
        vector_metadata = is_q5 and self.state.spec.decode.uses_vector_metadata_load
    ...
    def _emit_q5_k_decode(self, asm: _Assembly, *, label_suffix: str = "") -> None:
        ...
        if self.state.spec.decode.hoists_bitfield_shift:
            self._emit_q45_nibble_shift(asm)
    ...
    def _emit_q5_k_decode_chunk(self, asm: _Assembly, chunk: int) -> None:
        ...
        low = r.global_read_b + 4 * row + first_element // 4
        high = r.global_read_b + 4 * decoder_rows + 4 * row + first_element // 4
        if not self.state.spec.decode.hoists_bitfield_shift:
            asm.inst(f"v_and_b32 v{t}, 1, v{self.physical.address.quant_shift}")
            asm.inst(f"v_lshlrev_b32 v{t}, 2, v{t}")
